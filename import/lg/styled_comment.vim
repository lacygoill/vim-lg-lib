...
def Fix_allbut(ft: string) #{{{2
    ...
    if !has_key(s:allbut_groups, ft)
        ...
        #}}}
        s:allbut_groups[ft] = execute('syn list')
            ->split('\n')
        ...
        #}}}
        filter(s:allbut_groups[ft], {_, v -> v =~ '^' .. ft})
    endif

    for group in s:allbut_groups[ft]
        let cmds = Get_cmds_to_reset_group(group)
